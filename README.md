# BYD AutoLock — iOS / watchOS

BYD 전기차 자동 잠금/해제 앱의 비공식 iOS 포팅 프로젝트입니다.
Android [BydAutoLock](https://github.com/PoorGrammerA/BydAutoLock)을 네이티브 Swift/SwiftUI로 완전 재구현했습니다.

> **비공식 프로젝트**입니다. BYD 자동차와 무관하며, 사용에 따른 책임은 사용자 본인에게 있습니다.

---

## 주요 기능

### iPhone 앱

| 기능 | 설명 |
|---|---|
| **자동 잠금 해제** | BLE RSSI가 임계값 이상이면 자동 잠금 해제 |
| **자동 잠금** | RSSI가 임계값 이하로 떨어지면 자동 잠금 |
| **예측 사전 해제** | RSSI 상승 기울기(선형 회귀)로 접근을 미리 예측, 도착 전 선제적으로 해제 |
| **잠금 결과 검증** | 자동 잠금/해제 후 35초 뒤 실제 차량 상태를 확인, 불일치 시 재시도 후 알림 |
| **신호 소실 보호** | BLE 신호 끊김 60초 후 안전 잠금 (30초 지연 알림, 복구 시 자동 취소) |
| **RSSI 필터링** | EMA 지수 이동 평균 + 이상값 제거(선형 회귀 기반) + 히스테리시스 |
| **주행 중 잠금 차단** | CoreMotion으로 주행 감지 시 자동 잠금 및 알림 스킵 |
| **차량 상태 조회** | 잠금 여부, 배터리 %, 주행 가능 거리, 실내 온도, 에어컨 상태 |
| **수동 제어** | 앱 내 잠금/해제 버튼 (결과 폴링으로 즉시 확인) |
| **에어컨 수동 제어** | 앱 내 에어컨 켜기/끄기 버튼 |
| **에어컨 자동 제어** | 잠금 해제 시 자동 켜기 / 잠금 시 자동 끄기 (온도·풍속·순환 설정 가능) |
| **지오펜싱** | 차량 이탈 시 BLE 스캔 자동 중단, 복귀 시 재개 (반경 50~500m 조정 가능) |
| **주차 위치 저장** | 주차 시 GPS 좌표 자동 기록, Apple 지도 연동 ("내 차 찾기") |
| **백그라운드 동작** | 앱 종료/화면 잠금 상태에서도 자동 잠금/해제 동작 |
| **15개 지역 서버** | KR / EU / JP / SG / AU / BR / MX / NO / UZ / KZ / IN / ID / VN / SA / OM |
| **테마 설정** | 라이트 / 다크 / 시스템 선택 가능 |
| **디버그 로그** | BLE·API·Geofence·GPS·Motion 이벤트 SQLite 저장, 태그 필터·공유 지원 |

### 홈 화면 / 잠금 화면 위젯

| 위젯 | 표시 내용 |
|---|---|
| Small (2×2) | 서비스 ON/OFF · 잠금 상태 · 배터리 % |
| Medium (4×2) | 서비스 · 잠금 · 배터리 · RSSI |
| 잠금 화면 원형 | 잠금 상태 아이콘 |
| 잠금 화면 직사각형 | 서비스 상태 · 잠금 상태 · 배터리 % |

### Apple Watch 앱

| 기능 | 설명 |
|---|---|
| **차량 상태 표시** | 서비스 ON/OFF · 잠금/열림 · 배터리 % |
| **수동 제어** | 잠금 해제 / 잠금 버튼 |
| **서비스 토글** | Watch에서 서비스 시작/중지 |
| **iPhone 연결 상태** | 연결 끊김 시 안내 표시 |

### Watch 페이스 컴플리케이션

| 종류 | 표시 내용 |
|---|---|
| 원형 (Circular) | 잠금 상태 아이콘 + 배터리 % |
| 직사각형 (Rectangular) | 서비스/잠금/배터리 상태 + 잠금·해제 버튼 |
| 코너 (Corner) | 잠금 아이콘 + ON/OFF 레이블 |

> 컴플리케이션에서 잠금/해제 버튼 탭 시 Watch 앱이 즉시 열리며 동작 실행

### 알림

| 알림 | 조건 |
|---|---|
| 잠금 / 잠금 해제 | 자동·수동 동작 완료 (결과 검증 후 발송) |
| 잠금 실패 | 검증 재시도 후에도 실제 차량 상태 불일치 시 |
| 차량 신호 끊김 | BLE 소실 30초 후 (복구 시 자동 취소) |
| 에어컨 켜기/끄기 | 자동·수동 에어컨 동작 완료 |
| 서비스 시작/중지 | 서비스 상태 변경 시 |
| 배터리 부족 | 차량 배터리가 설정값 이하일 때 (기준 5~50% 조정 가능) |
| 작동 비밀번호 미설정 | BYD 앱에서 PIN을 설정하지 않은 경우 |
| 자동 동작 일시 중단 | 짧은 시간 안에 자동 동작이 과다 반복될 때 |

---

## 기술 스택

| 영역 | 사용 기술 |
|---|---|
| UI | SwiftUI (Dark theme, NavigationStack) |
| BLE | CoreBluetooth (Background Central, State Restore) |
| 위치 | CoreLocation (CLCircularRegion, 지오펜싱) |
| 모션 | CoreMotion (CMMotionActivityManager) |
| 암호화 | CommonCrypto (AES-128-CBC) · CryptoKit (MD5/SHA1) · Bangcle 커스텀 코덱 |
| 네트워크 | URLSession async/await (Swift Concurrency) |
| 저장소 | UserDefaults · Keychain (Security.framework) · SQLite3 |
| Watch 연동 | WatchConnectivity (WCSession) |
| 위젯/컴플리케이션 | WidgetKit |

---

## 아키텍처

```
iPhone App
├── AutoLockService          ← BLE 스캔 + RSSI 처리 + API 호출 (@MainActor)
├── BydVehicleService        ← BYD Cloud API 클라이언트 (actor)
├── GeofenceManager          ← CoreLocation 지오펜싱
├── WatchConnectivityManager ← Watch ↔ iPhone 양방향 통신
├── StorageManager           ← Keychain + UserDefaults + App Group
└── LogManager               ← SQLite 디버그 로그

Watch App
├── WatchMainView            ← SwiftUI UI
├── WatchConnectivityManager ← iPhone 명령 전송 / 상태 수신
└── WatchComplication        ← WidgetKit 컴플리케이션

Widget Extension
└── BydAutoLockWidget        ← WidgetKit (App Group으로 데이터 공유)
```

---

## 빌드 방법

### 요구 사항

- Xcode 15 이상
- iOS 16+ / watchOS 10+ 실기기
- Apple 계정 (무료 계정으로 사이드로딩 가능)
- `xcodegen` (`brew install xcodegen`)

### 빌드 단계

```bash
# 1. 저장소 클론
git clone https://github.com/GeyuongGongPark/BydAutoLock-iOS.git
cd BydAutoLock-iOS

# 2. project.yml에서 Bundle ID 변경
# PRODUCT_BUNDLE_IDENTIFIER: com.yourname.bydautolock
# App Group: group.com.yourname.bydautolock (iPhone + Widget entitlements)

# 3. Xcode 프로젝트 생성
xcodegen generate

# 4. Xcode로 열기
open BydAutoLock.xcodeproj

# 5. Product → Archive
```

### 무료 계정 사이드로딩 (AltStore / Sideloadly)

```bash
# Archive 후 IPA 추출
ARCHIVE=$(ls -t ~/Library/Developer/Xcode/Archives/**/*.xcarchive | head -1)
mkdir -p Payload
cp -r "$ARCHIVE/Products/Applications/BYD AutoLock.app" Payload/
zip -r BydAutoLock.ipa Payload
```

1. AltStore / Sideloadly로 iPhone에 설치
2. **설정 → 일반 → VPN 및 기기 관리**에서 개발자 신뢰

> 무료 계정은 7일마다 재서명 필요. 슬롯 3개 한도.

---

## 초기 설정

1. 앱 실행 후 **설정 → BYD 계정 설정**에서 이메일/비밀번호/PIN/지역 입력 후 로그인
2. **설정 → 블루투스 기기 설정**에서 차량 BLE 기기 선택
3. **설정 → RSSI 임계값 설정**에서 임계값 조정 (기본: 잠금 해제 -70dBm / 잠금 -85dBm)
4. 메인 화면에서 **자동 잠금 서비스 토글 ON**

---

## iOS 백그라운드 BLE 제한 안내

| 항목 | Android | iOS |
|---|---|---|
| 백그라운드 광고 스캔 | 무제한 | 시스템이 주기적으로 전달 (빈도 제한) |
| AllowDuplicatesKey | 항상 유효 | 백그라운드에서 무시됨 |
| 앱 종료 후 복원 | Foreground Service | `CBCentralManagerOptionRestoreIdentifierKey` |

포그라운드에서는 Android와 동일하게 동작합니다.

---

## 프로젝트 구조

```
BydAutoLock/
├── API/
│   ├── BangcleCodec.swift           # BYD 독자 CBC-AES 코덱 (808KB 룩업 테이블)
│   ├── CryptoUtils.swift            # MD5, SHA1-Mixed, AES-128-CBC
│   ├── BydConfig.swift              # 15개 지역 서버 팩토리
│   └── BydVehicleService.swift      # BYD API async/await 클라이언트
├── Models/                          # VehicleStatus, GpsInfo, HvacStatus 등
├── Service/
│   ├── AutoLockService.swift        # 핵심 서비스 (BLE + RSSI + API)
│   ├── GeofenceManager.swift        # CoreLocation 지오펜싱
│   └── WatchConnectivityManager.swift
├── Storage/
│   ├── StorageManager.swift         # UserDefaults + Keychain + App Group
│   └── KeychainHelper.swift
├── Log/
│   ├── LogManager.swift             # SQLite 로그 (최대 5,000행)
│   └── LogEntry.swift
├── Views/                           # SwiftUI 뷰 (메인, 설정, 로그)
└── Resources/
    └── bangcle_tables.bin           # BYD 암호화 룩업 테이블 (808KB)

BydAutoLockWatch/
├── BydAutoLockWatchApp.swift        # Watch 앱 진입점 + 딥링크 처리
├── WatchMainView.swift              # Watch UI
├── WatchConnectivityManager.swift
└── WatchComplication.swift          # WidgetKit 컴플리케이션

BydAutoLockWidget/
└── BydAutoLockWidget.swift          # iOS 홈/잠금 화면 위젯
```

---

## 릴리즈 노트

### v1.5.9 (build 22) — 2026-08-19

**버그 수정 (로그 분석 + 화이트박스 테스트)**

- **주행 후 귀가 시 자동 해제 미발동 수정** — 주행 중 지오펜스 진입 시 BLE 재개가 차단된 상태에서, 주행 종료 후 즉시 BLE를 재개하지 않아 차 앞에 서 있어도 unlock이 발동하지 않던 버그 수정
- **서비스 재시작 시 지오펜스 상태 오염 수정** — `stop()` 후 `isInsideGeofence`가 리셋되지 않아 재시작 시 지오펜스 외부인데도 BLE 스캔이 시작되던 버그 수정
- **자동 잠금 2회 연속 실패 후 unlock 영구 차단 수정** — 잠금 API가 2회 모두 실패했을 때 `lastKnownLocked`가 "잠김"으로 유지되어 이후 차에 접근해도 unlock이 발동하지 않던 버그 수정
- **블루투스 재활성화 시 불필요한 모션 감지 재등록 수정** — 블루투스를 껐다 켤 때 정지 감지 타이머가 불필요하게 재시작되던 버그 수정
- **재연결 즉시 unlock 이중 발동 수정** — 예측 해제 진행 중 BLE 재연결 시 unlock API가 중복 호출되던 버그 수정
- **잠금 직후 재연결로 즉시 열리는 버그 수정** — 자동 잠금 성공 후 BLE 재연결 시 raw RSSI 1샘플만으로 즉시 unlock이 발동되던 버그 수정. 잠금 상태가 확인된 경우 EMA 경로(다수 샘플)로 위임하도록 변경

---

### v1.5.8 (build 21) — 2026-08-18

**개선 (로그 분석 기반)**

- **SEALION 7 BLE 파라미터 조정** — 4일치 실사용 로그 분석 결과 반영
  - 신호 소실 안전 잠금 대기 시간 60초 → 90초: 주차 직후 BLE 재연결 불규칙으로 인한 오잠금 방지
  - 예측 해제 최소 기울기 0.5 → 0.8 dBm/s: SEALION 7 접근 기울기 범위(최대 2.7 dBm/s)에 맞게 노이즈 필터링 강화

---

### v1.5.7 (build 20) — 2026-08-14

**버그 수정 (실사용 피드백 반영)**

- **재설치 후 초기에만 문이 열리는 버그 수정** — 첫 잠금 해제 성공 후 `lastKnownLocked = false`로 기록된 상태가 이후에도 유지되어, BYD 앱/수동 키로 잠근 뒤 다시 접근해도 "이미 열린 상태"로 판단해 unlock API를 호출하지 않던 버그 수정
  - 지오펜스 재진입 시 잠금 상태를 초기화해 새 접근 세션으로 처리
  - BLE 신호가 2분 이상 끊겼다가 재연결되면 새 접근 세션으로 처리
  - ATTO 3의 20초 BLE 재연결 사이클에는 영향 없음

---

### v1.5.6 (build 19) — 2026-08-12

**개선 (사용자 피드백 반영)**

- **자동 잠금/해제 결과 검증 추가** — "알림은 오는데 실제 문이 열리거나 잠기지 않는" 현상 개선. 자동 잠금/해제 명령 전송 후 35초 뒤 실제 차량 상태를 조회해 결과를 검증. 불일치 시 자동 재시도 후 45초 뒤 재검증
  - 검증 성공 시: 기존과 동일한 잠금/해제 완료 알림 발송
  - 최종 실패 시: "잠금 실패 - 차량 상태를 확인해주세요" 알림 발송
  - 수동 제어는 기존과 동일 (폴링으로 즉시 결과 확인)
  - 차량 상태 조회 자체가 실패할 경우 명령 전송 기준으로 알림 (보수적 처리)

---

### v1.5.5 (build 18) — 2026-07-09

**버그 수정 (실사용 로그 분석 기반 — SEALION 7 미동작 원인 대응)**

- **자동 잠금/해제 후 상태 오표시 수정** — 재시도 코드에서 `try?`로 에러를 무시한 채 무조건 `lastKnownLocked`를 업데이트하던 버그 수정. 실패해도 앱에서 성공으로 표시되던 문제 해결. `do-catch`로 교체하여 성공 시에만 상태 업데이트
- **5011 에러 알림 추가** — 작동 비밀번호(PIN) 미설정 상태에서 모든 원격 제어 명령이 거부(5011)되는 경우, 불필요한 재시도 없이 즉시 종료하고 "BYD 앱에서 작동 비밀번호를 설정해주세요" 알림 발송
- **이탈 감지 시 API 미발동 수정** — RSSI 폴링 Background Task 만료 시점에 이탈이 감지되면 새로 생성된 Task가 앱 suspend로 API를 실행하지 못하던 문제 수정. `triggerCarAction` 내부에 전용 BG Task를 추가하여 API 실행 완료까지 suspend 차단
- **주행 중 안전 잠금 오발 수정** — 탑승 직후 주행 시작 시 `signalLossTimer`가 이미 실행 중이면 `isDriving` 전환 후에도 타이머가 만료되어 안전 잠금이 발동되던 문제 수정. 타이머 핸들러에 `isDriving` 체크 추가
- **자동 해제 실패 시 재시도 추가** — 자동 잠금에만 있던 재시도 로직을 자동 해제에도 추가. 해제 실패 시 2초 후 1회 재시도 (6024 서버 상태 불일치 대응)

**개선**

- **차량별 BLE 보정값 구조(VehicleProfile) 추가** — DOLPHIN / ATTO 3 / SEALION 7 차종별로 `rssiWindowDuration`, `signalLossGracePeriod`, `predictiveMinSlope` 파라미터를 독립적으로 관리. 현재는 동일한 기본값, 로그 수집 후 데이터 기반으로 차종별 보정 예정
- **에어컨 자동 시작 로그에 풍속 값 추가** — "에어컨 자동 시작: 22.0°C, 풍속: 2단 → 성공" 형태로 실제 전송값 기록. windLevel 관련 문제 추적 용이

---

### v1.5.4 (build 17) — 2026-07-08

**버그 수정 (실사용 로그 분석 기반)**

- **에어컨 자동 시작 미동작 수정** — `try?`로 에러를 조용히 무시하여 실패 여부를 확인할 수 없던 문제 수정. `do-catch`로 교체하여 실패 시 에러 메시지를 로그에 기록
- **에어컨 연속 명령 6024 오류 수정** — 잠금 해제 API 완료 직후 바로 에어컨 API를 호출하면 서버가 이전 명령 처리 중이라 6024 에러를 반환하던 문제 수정. 에어컨 호출 전 2초 딜레이 추가

---

### v1.5.3 (build 15) — 2026-07-06

**버그 수정 (실사용 로그 분석 기반)**

- **신호 소실 오탐 알림 수정** — 차량 근처(BLE 재연결 사이클 중)이거나 주행 시작 직후 isDriving 감지 지연 구간에서 "차량 신호 끊김" 알림이 발송되던 문제 수정. 알림을 30초 지연 발송으로 변경 → 30초 내 신호 복구 시 자동 취소
- **신호 복구 시 알림 쿨다운 리셋 누락 수정** — pending 알림이 취소됐음에도 5분 쿨다운이 유지되어 이후 실제 신호 소실 시 알림을 못 받던 문제 수정
- **서비스 중지 시 pending 알림 미취소 수정** — 서비스를 끈 후 30초 뒤 "차량 신호 끊김" 알림이 발송되던 문제 수정

**개선**

- **차량 상태 새로고침 속도 개선** — 차량 tbox 응답 대기 구조 개선. request 직후 1.5초 대기 후 폴링 시작 → 첫 번째 시도 성공 가능성 향상
- **신호 소실 로그/알림 메시지 개선** — "60초 후 잠금 예정(확정)" 표현 → "60초 내 미복구 시 안전 잠금(조건부)" 표현으로 변경. 신호 복구 시 취소된다는 의미가 명확히 전달되도록

---

### v1.5.2 (build 14) — 2026-07-06

**버그 수정 (실사용 로그 분석 기반)**

- **RSSI "신호 없음" UI 깜빡임 방지** — BLE 재연결 사이클(20초)마다 RSSI 카드가 "신호 없음"으로 깜빡이던 문제 수정. rawRssi/smoothedRssi nil 처리를 5초 지연 타이머로 처리
- **복호화 실패 자동 복구** — 세션 갱신 직후 CryptoError 발생 시 자동 재로그인 후 재시도. 간헐적 "오류" 표시 방지
- **차량 상태 조회 재시도 강화** — vehicleRealTimeResult 폴링 1회 → 최대 5회(2초 간격). tbox 응답 지연(3002) 대응

---

### v1.5.1 (build 13) — 2026-07-03

**버그 수정 (실사용 로그 분석 기반)**

- **주행 중 자동 잠금 오발 수정** — 주행 중 RSSI 급락 시 이탈 감지 경로에서 `isDriving` 체크 누락으로 잠금 API가 실행되던 문제 수정
- **신호 소실 알림 폭탄 수정** — BLE 20초 재연결 사이클마다 "60초 후 잠금 예정" 알림이 반복 발송되던 문제 수정. 알림 쿨다운 1분 → 5분 + 재연결 시 쿨다운 리셋 제거
- **예측 사전 해제 취소 후 오동작 수정** — RSSI 하강으로 예측 해제 취소 중 API 응답이 도착한 경우 `lastKnownLocked` 상태가 잘못 업데이트되던 문제 수정
- **Motion 중복 콜백 수정** — `startMotionUpdates()` 재호출 시 이전 manager를 정리하지 않아 "움직임 감지" 로그가 연속 중복 발생하던 문제 수정
- **로그 파일에 앱 버전 포함** — 서비스 시작 시 `[App] BYD AutoLock vX.X (build N)` 로그 자동 기록. 내보내기 파일 첫 줄에 버전 헤더 추가

---

### v1.5 (build 12) — 2026-07-03

**라이트/다크/시스템 테마 설정 추가**
- 메뉴 → 기타 → 테마에서 라이트 / 다크 / 시스템 중 선택 가능
- 기본값: 시스템 (iOS 설정 따라감)
- 선택한 테마는 앱 재시작 후에도 유지됨

---

### v1.4.2 (build 11) — 2026-07-01

**iOS 26/27 베타 백그라운드 BLE 안정화**
- 앱이 백그라운드일 때 BLE 장치를 수 시간 동안 못 찾던 문제 수정
  → 기존: 전체 스캔(`scanForPeripherals`) 시도 → iOS 백그라운드에서 차단됨
  → 변경: UUID 저장 후 `retrievePeripherals` + `connect()` 로 스캔 없이 직접 재연결
  → 실측: 9시간 공백 → 38분으로 개선

**자동 해제 반응 속도 개선 (예측 사전 해제)**
- 차에 접근해도 해제까지 1~3분 걸리던 문제 개선
  → ATTO 3 BLE 20초 끊김/재연결 사이클에서 RSSI 데이터가 매번 초기화되던 문제
  → 60초 시간 기반 누적으로 변경, 여러 사이클에 걸쳐 접근 기울기 계산 가능

**이탈 시 자동 잠금 누락 수정**
- 해제 직후 BLE 신호가 순간 급락할 때 잠금이 실행되지 않던 문제 수정
  → 해제 후 30초 쿨다운 중 이탈 감지 시 쿨다운 만료 후 자동 잠금 예약
  → 그 사이 다시 접근하면 예약 취소

**안전성 개선 (화이트박스 테스트 #4)**
- 주행 시작 시 이탈 예약 잠금 자동 취소 (주행 중 오발 방지)
- 정지 감지 중 BLE 끊김 시 불필요한 재연결 시도 차단
- 복호화 실패 시 빈 문자열 대신 에러 반환으로 변경

---

### v1.4.1 (build 10) — 2026-06-30

**로그 전체 누락 버그 수정**
- v1.4 이전 일부 기기에서 로그가 전혀 기록되지 않던 문제 수정
  → SQLite DB 파일 경로의 디렉토리가 자동 생성되지 않아 DB 열기 실패

**안정성 개선**
- 자동 잠금 API 실패 후 45초 재시도 성공 시 잠금 상태 미반영 버그 수정
  → 이후 재연결 사이클에서 중복 잠금 API 호출 방지
- 차량 연결 없이 상태 새로고침 시 "로그인이 필요합니다" → "차량과 연결해 주세요"로 문구 수정
- 로그 뷰 빈 상태 안내 문구 오류 수정 (v1.3에서 제거된 기능 언급 잔존)

---

### v1.4 (build 9) — 2026-06-29

**주행 관련 버그 수정**
- 주행 종료 후 자동 잠금이 실행되지 않던 버그 수정
  → 지오펜스 이탈 후 주행 종료 시 즉시 GPS 폴링 + 잠금 API 호출
- 주행 중 지오펜스가 반복적으로 "내부"로 감지되던 버그 수정
  → BYD GPS API가 실시간 speed를 반환하지 않아 주행 중에도 지오펜스 재등록되던 문제
  → CoreMotion `isDriving` 기준으로 주행 중 지오펜스 재등록/진입 이벤트 차단

**백그라운드 안정화**
- GPS 정확도를 `ThreeKilometers`(셀룰러 기지국 기반)로 낮춰 배터리 소모 절감 (앱 suspend 방지 효과 동일)
- `fetch` 백그라운드 모드 선언 제거 (구현 없는 dead 선언)

**로그 스팸 제거**
- BLE 20초 재연결 사이클마다 반복되던 "이미 닫힘/열림 상태 - 명령 스킵" 로그 제거
  → 접근/이탈 감지 단계에서 `lastKnownLocked` 사전 필터링으로 불필요한 API 호출 차단

**코드 안전성 개선 (화이트박스 테스트 #3)**
- 타이머 재시작 시 cancel 누락 수정 (`sessionRefreshTimer`, `gpsPollTimer`)
- 선형 회귀 함수 force unwrap 제거 (`points.first!`, `points.last!`)
- `URL(string:)!` force unwrap 제거 → throw로 안전 처리
- AES 암호화/복호화 실패 시 빈 문자열 반환 → throws로 변경
- SQLite open/prepare 에러 체크 및 `db nil` guard 전반 적용
- BangcleCodec 테이블 로딩 bounds check, PKCS7 패딩 전체 바이트 검증
- `DateFormatter` 매 호출마다 생성되던 문제 → static으로 변경
- 신호 복구 시 신호 소실 알림 쿨다운 리셋

---

### v1.3 (build 8) — 2026-06-28

**안정성 개선**
- 주행 중 BLE 신호 끊김 시 알림 및 자동 잠금 스킵 (주행 중 오발 방지)
- 자동 잠금 API 실패(차량 통신 오류 등) 시 45초 후 1회 자동 재시도
- 움직임 감지(CoreMotion) 중복 콜백으로 BLE 스캔이 수십 번 반복 시작되던 버그 수정
- 타이머 재시작 시 기존 타이머 누수 방지
- 신호 소실 grace timer가 취소된 후에도 잠금이 실행되던 edge case 수정

**설정 개선**
- RSSI 임계값 역전(잠금 해제값 ≤ 잠금값) 저장 시 오류 표시 및 차단
- 지오펜스 반경 setter 범위 검증 추가 (50~500m 강제 클램핑)

**로그**
- 디버그 로깅 토글 제거 — 항상 로그 기록 (설정 메뉴 단순화)
- 신호 끊김 알림 문구 개선: "60초 후 자동으로 잠금됩니다"

**코드 품질**
- 미사용 변수(rssiLogTimer, rssiSamples) 제거
- silentReLogin 무한 재귀 방지 (isRelogging 플래그)

---

### v1.2.2 (build 7) — 2026-06-26

**백그라운드 안정화**
- iOS가 앱을 suspend할 때 BLE RSSI 폴링이 멈추던 문제 해결
  → `startUpdatingLocation()` (10m 정확도)으로 백그라운드 실행 유지
- BG 작업(UIBackgroundTask) 만료로 BLE가 끊길 때 오발 잠금 방지

**지오펜스 개선**
- GPS 갱신 시 지오펜스 진입 이벤트 중복 발생 수정 (2초 디바운스 + 동일 좌표 재등록 방지)
- 지오펜스 이탈 상태에서 불필요한 BLE 재연결 시도 차단
- **지오펜스 반경을 설정에서 직접 조정 가능** (50~500m, 기본 150m)

**버그 수정**
- 로그 분류 필터 바가 가려지는 문제 재수정 (재발 방지)
- Watch App Group 강제 언래핑 크래시 방지
- LogManager 멀티스레드 안전성 확보

---

### v1.2.1 (build 6) — 2026-06-26

- 위젯 App Group 권한 추가 및 표시 항목 개선
- 로그 필터에 BG 태그 추가

### v1.2 (build 5) — 2026-06-25

- 지오펜스 내부에서 서비스 시작 시 스캔 차단 버그 수정
- dead code 제거 및 stop() 상태 초기화

---

## 참고 / 원본 프로젝트

- Android 원본: [PoorGrammerA/BydAutoLock](https://github.com/PoorGrammerA/BydAutoLock)
- BYD API 분석: [jkaberg/pyBYD](https://github.com/jkaberg/pyBYD)
