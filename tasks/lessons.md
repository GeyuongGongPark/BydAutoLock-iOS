...
**"기능 추가하자"는 요청이 어떤 레이어에 대한 건지 먼저 확인할 것:**
- "테마 기능이나 추가하자" → 직전 대화가 랜딩 페이지 작업이었다고 해서 랜딩 페이지에 추가한 것은 오판
- 실제 의도: iOS 앱에 라이트/다크/시스템 테마 설정 추가
- 시뮬레이터 빌드 후 "기능 안보임"으로 확인됨
...
**직접 확인하지 않은 외부 제품의 동작을 단정적으로 설명하지 말 것:**
- 현대 블루링크 "원격 시동"을 "에어컨/히터만 켜는 pre-conditioning"으로 잘못 설명
- 실제로는 엔진 시동이 걸리고 P 상태에서 공회전, 탑승 후 주행 가능 (10분 시간 제한만 있음)

**원칙:**
- 직접 확인한 사실 → 단정적으로 말 가능
- 추측/간접 지식 → "~로 알고 있습니다", "확인이 필요합니다"로 명시
- 틀린 정보를 전달하는 것보다 모른다고 하는 게 낫다
...
## 자동 에어컨 API 연속 호출 실패 패턴

**증상**: 로그에 "에어컨 자동 시작" 찍히지만 실제로 에어컨 동작 안 함. 수동은 됨.

**원인 1 — try? 무시로 에러 미기록**: `try?`로 에러를 조용히 버려 실패 여부를 로그로 확인 불가.
→ `do-catch`로 변경하여 실패 시 에러 메시지를 로그에 기록해야 함.

**원인 2 — 연속 명령 처리 충돌 (6024)**: 잠금/해제 API 완료 직후 바로 에어컨 API 호출 시 서버가
이전 명령 처리 중이라 6024("마지막 작업이 완료되지 않았습니다") 에러 반환 가능.
→ 에어컨 호출 전 2초 딜레이 추가.

**원칙**: 연속 API 호출 시 BYD 서버가 이전 명령을 완전히 처리할 시간이 필요. 잠금/해제 직후 바로 다른 명령 금지.
